...
export function Marquee({
  ...
  const fadeFrom = (from === "background" ? "from-background" : "from-card");
...
<>
  <div
    className={cn(
      ...
      vertical
        ? `${fadeFrom} inset-x-0 top-0 h-1/6 bg-gradient-to-b`
        : `${fadeFrom} inset-y-0 left-0 w-1/6 bg-gradient-to-r`
    )}
  />
  <div
    className={cn(
      "pointer-events-none absolute z-10",
      vertical
        ? `${fadeFrom} inset-x-0 bottom-0 h-1/6 bg-gradient-to-t`
        : `${fadeFrom} inset-y-0 right-0 w-1/6 bg-gradient-to-l`
    )}
  />
</>      )}
      {Array(repeatCount)
